...
const muncities = rawMuncities;
const munCityIndex = muncities.reduce((acc, m) => {
    var _a;
    (acc[_a = m.provCode] ?? (acc[_a] = [])).push(m);
    return acc;
}, {});
// Dynamically collect NCR provCodes (regCode 13, skip the fake 000 entry)
const NCR_PROVCODES = Array.from(new Set(muncities.filter((m) => m.regCode === "13").map((m) => m.provCode)));
/**************************************************************************************
 ...
 * ```
 **************************************************************************************/
export function listMuncities(provCode) {
    if (!provCode)
        return [...muncities];
    if (provCode === "000") {
...
//# sourceMappingURL=muncities.js.map